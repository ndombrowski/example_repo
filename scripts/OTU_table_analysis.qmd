...
Next, we read in some custom functions that allow us to, among others print summary statistics for an OTU table, functions to plot the same kind of plot multiple times, or a custom theme for plotting.
...
```{r,message=FALSE}
# Raw data 
plot_otu_histogram(physeq_filt, metadata_combined, bins = 100, title_suffix = "(raw)")
```

```{r,message=FALSE}
# Relative abundance data 
plot_otu_histogram(physeq_rel_abundance, metadata_combined, bins = 100, title_suffix = "(relative abundance)")
```

```{r,message=FALSE}
# Rarefied data 
plot_otu_histogram(physeq_rarified, metadata_combined, bins = 100, title_suffix = "(rarefied)")
```

```{r,message=FALSE}
# CLR data 
plot_otu_histogram(physeq_clr, metadata_combined, bins = 100, title_suffix = "(CLR)")
```
